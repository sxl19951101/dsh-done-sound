...
var LOGS_ROOT = join(dirname(fileURLToPath(import.meta.url)), "..", "logs");
function datedLogFileName(now = /* @__PURE__ */ new Date()) {
  const y = now.getFullYear();
  const m = String(now.getMonth() + 1).padStart(2, "0");
  const d = String(now.getDate()).padStart(2, "0");
  return `${y}${m}${d}-dsh-done-sound.log`;
}
function datedLogPath(now = /* @__PURE__ */ new Date()) {
...
var SCENES = ["normal", "interrupt", "error", "pending", "retry"];
var DEFAULT_AUDIO_FILES = {
...
var DEFAULT_AUDIO_PATHS = Object.fromEntries(
  Object.entries(DEFAULT_AUDIO_FILES).map(([scene, file]) => [
    scene,
    join(dirname(fileURLToPath(import.meta.url)), "..", "assets", file)
  ])
);
var DEFAULT_AUDIO_URLS = Object.fromEntries(SCENES.map((s) => [s, `/dsh-done-sound/audio/default-${s}`]));
...
var SoundMetaSchema = Schema.object({
  fileId: Schema.string().default(""),
  fileName: Schema.string().default(""),
  mime: Schema.string().default(""),
  size: Schema.number().default(0)
}).default({});
var TurnChimeSchema = Schema.object({
  ...
  playOnRetry: Schema.boolean().default(true),
...
var NS = "dsh-done-sound";
var ROUTE_PREFIX = "/dsh-done-sound";
var FILE_ID_RE = /^[A-Za-z0-9-]+$/;
function apply(ctx) {
  ...
  const readJsonBody = (req) => new Promise((resolve2, reject) => {
    const chunks = [];
    ...
    try {
      ...
      if (pathname === "/dsh-done-sound/api/log" && method === "POST") {
        ...
        if (message) {
          ...
          writeLog(body.source === "client" ? `CLIENT-${level}` : level, message.slice(0, 4e3));
        }
