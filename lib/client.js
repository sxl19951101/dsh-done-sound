  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      const TurnChimeDetector = (props) => {
        ...
        const lastAutoRetryRef = react.useRef(0);
        ...
        react.useEffect(() => {
          ...
          // Auto-retry: only when the model exhausted its own retries and the
          // turn ended with a real 'error' reason (internal llm/retry never
          // reaches here). Send a "继续" prompt so the agent resumes; a 60s
          // cooldown prevents retry loops.
          if (reason === 'error' && cfg.enabled && cfg.autoRetryOnError && snap.sessionId) {
            const now = Date.now();
            if (now - lastAutoRetryRef.current >= 60000) {
              ...
              try {
                ...
                if (remote && remote.sessions && typeof remote.sessions.prompt === 'function') {
                  ...
                  remote.sessions
                    .prompt({ sessionId: snap.sessionId, mode: 'queue', content: [{ type: 'text', text: '继续' }], clientTimeZone: tz })
                    .catch(() => {});
                  sent = true;
                }
              } catch {
              ...
              if (sent) setConfig({ lastEvent: { at: Date.now(), kind, reason, play: played, retried: true }, retryCooldownUntil: Date.now() + 60000 });
            }
          }
        }, [snap, cfg]);

        // Visible countdown chip in the session header while the auto-retry
        // cooldown is active (so the user sees it without opening settings).
