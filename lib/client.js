...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const css = [
      ...
      '.dtc-retry-chip{display:inline-flex;align-items:center;gap:4px;border:1px solid var(--dsw-alias-border-l2);border-radius:999px;padding:1px 8px;font-size:11px;color:var(--dsw-alias-label-secondary);white-space:nowrap;font-variant-numeric:tabular-nums}',
    ].join('');
    ...
    const COPY = {
      ...
      en: {
        ...
        retryCooldown: 'Auto-retry in {s}s',
        version: 'Version',
    ...
    function apply(ctx) {
      ...
      const TurnChimeDetector = (props) => {
        ...
        // Armed auto-retry: { deadline, sessionId, endTurnAtError } | null.
        // Sent only when the 60s window expires without the session recovering.
        ...
        react.useEffect(() => {
          ...
          if (pendingRetryRef.current !== null) {
            ...
            const cancel =
              snap.sessionId !== retry.sessionId ||
              snap.streaming === true ||
              snap.endTurn > retry.endTurnAtError ||
              snap.pendingCount > 0;
            if (cancel) {
              pendingRetryRef.current = null;
              setConfig({ retryCooldownUntil: null });
          ...
          // Arm the deferred auto-retry: after a real 'error' end, wait a 60s
          // window; if the session recovers during the window (streaming, new
          // turn, approval wait) the retry is cancelled above, otherwise it
          // fires when the window expires.
          if (reason === 'error' && cfg.enabled && cfg.autoRetryOnError && snap.sessionId) {
            if (pendingRetryRef.current === null) {
              const deadline = Date.now() + 60000;
              pendingRetryRef.current = { deadline, sessionId: snap.sessionId, endTurnAtError: snap.endTurn };
              setConfig({ retryCooldownUntil: deadline });
              setConfig({ lastEvent: { at: Date.now(), kind, reason, play: played, retryPending: true } });
            }
          }
        }, [snap, cfg]);

        // Visible countdown chip in the session header while an auto-retry
        // window is pending; fires the "继续" once the window expires.
        const [nowTick, setNowTick] = react.useState(Date.now());
        react.useEffect(() => {
          const timer = setInterval(() => {
            setNowTick(Date.now());
            const retry = pendingRetryRef.current;
            if (retry !== null && Date.now() >= retry.deadline) {
              pendingRetryRef.current = null;
              setConfig({ retryCooldownUntil: null });
              try {
                const remote = ctx && ctx.remote;
                if (remote && remote.sessions && typeof remote.sessions.prompt === 'function') {
                  const tz = typeof Intl !== 'undefined' && Intl.DateTimeFormat ? Intl.DateTimeFormat().resolvedOptions().timeZone : 'UTC';
                  remote.sessions
                    .prompt({ sessionId: retry.sessionId, mode: 'queue', content: [{ type: 'text', text: '继续' }], clientTimeZone: tz })
                    .catch(() => {});
                  setConfig({ lastEvent: { at: Date.now(), kind: 'error', reason: 'error', play: true, retried: true } });
                }
              } catch {
                // ignore
              }
        ...
        if (cooldownLeft > 0) {
          return react.createElement(
            'span',
            { className: 'dtc-retry-chip', title: '自动重连冷却中' },
            react.createElement('span', null, '🔄'),
            react.createElement('span', null, cooldownLeft + 's'),
          );
        }
        return null;
      };
