      ...
      Promise.resolve(prompt({ sessionId: sid, mode: 'queue', content: [{ type: 'text', text: '继续' }], clientTimeZone: tz }))
        .then((res) => {
          ...
          if (result && result.ok) {
            ...
            // Play the dedicated "auto-retry ok" sound when enabled.
            try {
              const view = configStore.value;
              if (view && view.enabled && view.playOnRetry !== false) playChime(view, 'retry');
            } catch {
    ...
    function apply(ctx) {
      ...
      const TurnChimeDetector = (props) => {
        ...
        react.useEffect(() => {
          ...
          const notifyPending = () => {
            const pend = () => (cfg.sounds && cfg.sounds.pending) || {};
            const canPlay = (view) => {
