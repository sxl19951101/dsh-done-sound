...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const css = [
      ...
      '.dtc-retry-chip{display:inline-flex;align-items:center;gap:6px;border:1px solid var(--dsw-alias-state-warn-primary,#f59e0b);border-radius:999px;padding:4px 12px;font-size:13px;font-weight:600;color:var(--dsw-alias-state-warn-primary,#f59e0b);background:rgba(245,158,11,.14);white-space:nowrap;font-variant-numeric:tabular-nums;box-shadow:0 0 0 1px rgba(245,158,11,.1)}',
      '.dtc-retry-chip .dtc-retry-ico{font-size:14px;line-height:1}',
    ].join('');
    ...
    function fireModuleRetry() {
      ...
      retryState.deadline = 0;
      try {
        const remote = pluginCtx && pluginCtx.remote;
        if (remote && remote.sessions && typeof remote.sessions.prompt === 'function') {
          ...
          remote.sessions
            .prompt({ sessionId: sid, mode: 'queue', content: [{ type: 'text', text: '继续' }], clientTimeZone: tz })
            .catch(() => {});
        }
      } catch {
        // ignore
    ...
    function apply(ctx) {
        ...
        return react.createElement(
          ...
          react.createElement(
            ...
            react.createElement('input', {
              ...
              max: 300,
              step: 5,
              value: cfg.retryDelaySeconds || 60,
              disabled: busy,
              onChange: (e) => {
                const v = Number(e.target.value);
                if (Number.isFinite(v) && v >= 10 && v <= 300) {
                  run('/dsh-done-sound/api/config', { retryDelaySeconds: Math.round(v / 5) * 5 });
                }
              },
            }),
      ...
      const TurnChimeDetector = (props) => {
        ...
        if (cooldownLeft > 0) {
          return react.createElement(
            'span',
            { className: 'dtc-retry-chip', title: '自动重连等待中，期间恢复则取消' },
