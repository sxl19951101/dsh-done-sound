...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const css = [
      ...
      '.dtc-scene{display:flex;align-items:center;gap:10px;min-height:30px}',
      '.dtc-scene-name{flex:0 0 112px;color:var(--dsw-alias-label-primary);font-weight:600;font-size:13px;white-space:nowrap}',
      '.dtc-scene-file{flex:1 1 auto;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:var(--dsw-alias-label-secondary);font-size:12px;text-align:left}',
      '.dtc-scene-btns{flex:0 0 auto;display:inline-flex;align-items:center;gap:6px}',
      '.dtc-scene-btns .dtc-btn{min-width:62px;text-align:center;padding:3px 8px}',
      '.dtc-switch-hint{flex:1 1 auto;text-align:left;padding-left:10px}',
      ...
      '.dtc-ok{color:var(--dsw-alias-state-success-primary);white-space:pre-wrap;font-size:12px;word-break:break-all}',
      ...
      '.dtc-retry-wrap{display:inline-flex;align-items:center;gap:6px;max-width:100%}',
      '.dtc-retry-cancel{border:1px solid #f59e0b;border-radius:6px;padding:5px 9px;color:#f59e0b;background:transparent;cursor:pointer;white-space:nowrap;font-size:12px}',
      '.dtc-retry-cancel:hover{background:rgba(245,158,11,.12)}',
    ...
    const COPY = {
      zh: {
        ...
        retrySent: '已自动发送「继续」',
        retrySentPlayed: '已自动发送「继续」，成功音效已播放',
        retryFailed: '自动发送「继续」失败',
        retryCancelled: '已取消自动重连',
        cancelRetry: '终止重试',
        logPath: '日志文件',
        exportLog: '导出日志',
        exportSaved: '日志已导出',
        exportSavedFile: '保存文件名：{name}',
        exportSavedFallbackHint: '未弹出保存窗口，文件已存到浏览器默认下载文件夹（通常 C:\\Users\\你的用户名\\Downloads）',
        exportLogPath: '原始日志路径：{path}（复制到资源管理器地址栏可打开）',
        exportLogPathUnknown: '原始日志路径未知（可在插件目录 logs 文件夹查看）',
        sceneNormal: '正常完成音',
        sceneInterrupt: '中断音',
        sceneError: '出错音',
        scenePending: '等待确认音',
        ...
        defaultSound: '（默认音）',
        retrySound: '重连成功也响',
        retrySoundHint: '自动发送「继续」成功后播放提示音（默认开启）',
        version: '当前版本',
      ...
      en: {
        ...
        retrySentPlayed: 'auto-sent "continue"; success sound played',
        retryFailed: 'failed to auto-send "continue"',
        retryCancelled: 'auto-retry cancelled',
        cancelRetry: 'Stop retry',
        logPath: 'Log file',
        exportLog: 'Export log',
        exportSaved: 'Log exported',
        exportSavedFile: 'Saved file name: {name}',
        exportSavedFallbackHint: 'No save dialog shown; saved to the browser default Downloads folder (usually C:\\Users\\YourName\\Downloads)',
        exportLogPath: 'Original log path: {path} (copy into File Explorer address bar to open)',
        exportLogPathUnknown: 'Original log path unknown (check the plugin logs folder)',
        sceneNormal: 'Completion sound',
        sceneInterrupt: 'Interrupt sound',
        sceneError: 'Error sound',
        scenePending: 'Approval-wait sound',
        sceneRetry: 'Auto-retry success sound',
        defaultSound: '(default)',
        retrySound: 'Play on retry success',
        retrySoundHint: 'Play a sound when "continue" is sent successfully (default on)',
        version: 'Version',
    ...
    const emptySounds = () =>
      Object.fromEntries(
        SCENES.map((s) => [s, { fileId: '', fileName: '', mime: '', size: 0, url: null, defaultUrl: null }]),
      );
    // Build the per-scene sounds map from a host status payload (reused by
    // syncConfig and by the detector's fresh-view re-fetches).
    const syncSoundsFrom = (payload, fallback) => {
      const out = (fallback && fallback.sounds && typeof fallback.sounds === 'object' ? { ...fallback.sounds } : emptySounds());
      const rawSounds = payload.sounds && typeof payload.sounds === 'object' ? payload.sounds : {};
      for (const scene of SCENES) {
        const s = rawSounds[scene] && typeof rawSounds[scene] === 'object' ? rawSounds[scene] : {};
        out[scene] = {
          fileId: typeof s.fileId === 'string' ? s.fileId : '',
          ...
          defaultUrl: typeof s.defaultUrl === 'string' && s.defaultUrl ? s.defaultUrl : null,
        };
      }
      return out;
    };
    ...
    function playChime(cfg, scene) {
      const s = (cfg.sounds && cfg.sounds[scene]) || {};
      const url = s.url || s.defaultUrl;
    ...
    function cancelModuleRetryFromCard() {
      if (!retryState.armed) return false;
      cancelModuleRetry();
      setConfig({
        retryCooldownUntil: null,
        lastEvent: { at: Date.now(), kind: 'retry', reason: 'cancelled', play: false, retried: false, retryCancelled: true },
      });
      return true;
    }
      ...
      Promise.resolve(prompt({ sessionId: sid, mode: 'queue', content: [{ type: 'text', text: '继续' }], clientTimeZone: tz }))
        .then((res) => {
          ...
          if (result && result.ok) {
            ...
            let retrySoundPlayed = false;
            try {
              const view = configStore.value;
              if (view && view.enabled && view.playOnRetry !== false) {
                const sound = (view.sounds && view.sounds.retry) || {};
                retrySoundPlayed = Boolean(sound.url || sound.defaultUrl);
                playChime(view, 'retry');
              }
            } catch {
            ...
            setConfig({
              retryCooldownUntil: null,
              lastEvent: { at: Date.now(), kind: 'retry', reason: 'retry-success', play: retrySoundPlayed, retried: true, retryFailed: false },
            });
          } else {
    ...
    function apply(ctx) {
      ...
      const syncConfig = (payload) => {
        ...
        const sounds = syncSoundsFrom(payload, null);
        setConfig({
          ...
          playOnRetry: payload.playOnRetry !== false,
      ...
      // 预热：模块一激活就拉取 Host 持久化配置（如自动重连秒数），
      // 避免页面刷新后倒计时/自动重连先用初始默认值（60s），
      // 而设置卡却显示已保存值（20s）——两者必须一致。
      // 失败静默，卡片挂载和 detector 挂载时还会再次同步兜底。
      api('/dsh-done-sound/api/status')
        .then((payload) => {
          if (payload && typeof payload === 'object') syncConfig(payload);
        })
        .catch(() => {});
      ...
      const TurnChimeCard = (props) => {
        ...
        // where to store it (filename is today's dated log name). After a
        // successful export, show a green banner with the saved file name and
        // the original log path so the user can find the file on Windows even
        // when the browser cannot show a save dialog.
        const exportLog = () => {
          setExportInfo(null);
          const showSaved = (savedName, fallbackHint) => {
            const lines = [L('exportSaved') + ' ✓', L('exportSavedFile').replace('{name}', savedName)];
            if (fallbackHint) lines.push(L('exportSavedFallbackHint'));
            // 原始日志路径来自 Host，复制到资源管理器地址栏即可直接打开，
            // 最方便排查「有概率无音效」类问题。
            lines.push(cfg.logPath ? L('exportLogPath').replace('{path}', cfg.logPath) : L('exportLogPathUnknown'));
            setExportInfo(lines.join('\n'));
          };
          fetch('/dsh-done-sound/api/log/export')
            ...
            .then((text) => {
              const fileName = cfg.logFileName || 'dsh-done-sound.log';
              const fallbackDownload = () => {
                ...
                showSaved(fileName, true);
              };
              // 优先用浏览器原生「另存为」对话框（File System Access API，
              // Chrome/Edge 支持），让用户自己选择保存位置；
              // 不支持或用户取消时回退为普通下载。
              if (typeof window.showSaveFilePicker === 'function') {
                window
                  ...
                  .then((handle) => {
                    const savedName = handle && typeof handle.name === 'string' && handle.name ? handle.name : fileName;
                    return handle.createWritable().then(async (writable) => {
                      await writable.write(text);
                      await writable.close();
                      showSaved(savedName, false);
                    });
                  })
                  .catch((err) => {
                    // 用户点了「取消」关闭对话框 → 不算错误，直接结束。
                    if (err && err.name === 'AbortError') return;
                    // 其他失败（如手势过期、浏览器限制）→ 回退为普通下载。
                    try {
                      if (typeof console !== 'undefined') console.warn('[dsh-done-sound] save picker failed, falling back to download:', err);
                    } catch {
                      // ignore
                    }
                    fallbackDownload();
        ...
        const onPick = (event, scene) => {
          ...
          reader.onload = async () => {
            ...
            await run('/dsh-done-sound/api/audio', { dataUrl, fileName: file.name, scene });
          };
        ...
        // One audio picker row per trigger scene. Each scene has its own
        // uploaded file (or falls back to its bundled default sound).
        const sceneRow = (scene) => {
          const s = (cfg.sounds && cfg.sounds[scene]) || {};
          return react.createElement(
            'div',
            { className: 'dtc-row dtc-scene' },
            react.createElement('span', { className: 'dtc-scene-name' }, L('scene' + scene[0].toUpperCase() + scene.slice(1))),
            react.createElement('span', { className: 'dtc-scene-file' }, s.fileName || L('defaultSound')),
            react.createElement(
              'span',
              { className: 'dtc-scene-btns' },
              react.createElement(
                'label',
                { className: 'dtc-btn dtc-file' },
                react.createElement('input', { type: 'file', accept: 'audio/*', disabled: busy, onChange: (e) => onPick(e, scene) }),
                L('pick'),
              ),
              react.createElement(
                'button',
                { className: 'dtc-btn', type: 'button', disabled: busy || !(s.url || s.defaultUrl), onClick: () => playChime(cfg, scene) },
                L('preview'),
              ),
              react.createElement(
                'button',
                { className: 'dtc-btn', type: 'button', disabled: busy || !s.fileId, onClick: () => run('/dsh-done-sound/api/clear', { scene }) },
                L('clear'),
              ),
            ),
          );
        ...
        return react.createElement(
          ...
          exportInfo !== null && react.createElement('div', { className: 'dtc-ok' }, exportInfo),

          SCENES.map((scene) => sceneRow(scene)),
          ...
          react.createElement(
            ...
            react.createElement('span', { className: 'dtc-muted dtc-switch-hint' }, L('interruptHint')),
          ),
          react.createElement(
            'div',
            { className: 'dtc-row' },
            ...
            react.createElement('span', { className: 'dtc-muted dtc-switch-hint' }, L('errorHint')),
          ),
          react.createElement(
            'div',
            { className: 'dtc-row' },
            ...
            react.createElement('span', { className: 'dtc-muted dtc-switch-hint' }, L('pendingHint')),
          ),
          react.createElement(
            'div',
            { className: 'dtc-row' },
            react.createElement(
              ...
              react.createElement('input', {
                type: 'checkbox',
                checked: cfg.playOnRetry,
                disabled: busy,
                onChange: (e) => {
                  run('/dsh-done-sound/api/config', { playOnRetry: e.target.checked });
                },
              }),
              react.createElement('span', null, L('retrySound')),
            ),
            react.createElement('span', { className: 'dtc-muted dtc-switch-hint' }, L('retrySoundHint')),
          ),
          react.createElement(
            'div',
            { className: 'dtc-row' },
            ...
            react.createElement('span', { className: 'dtc-muted dtc-switch-hint' }, L('autoRetryHint')),
          ),
          react.createElement(
            'div',
            { className: 'dtc-row' },
                ...
                (cfg.lastEvent.kind === 'interrupt'
                  ...
                  : cfg.lastEvent.kind === 'error'
                    ...
                    : cfg.lastEvent.kind === 'pending'
                      ? L('evPending')
                      : cfg.lastEvent.kind === 'retry'
                        ? L('evRetry')
                        : L('evNormal')) +
                ' → ' +
                (cfg.lastEvent.retryCancelled
                  ? L('retryCancelled')
      ...
      const TurnChimeDetector = (props) => {
        ...
        // 兜底：挂载时若配置尚未从 Host 同步（如页面刚刷新、apply 预热未完成），
        // 主动拉取一次，确保自动重连倒计时使用用户保存的秒数而非初始默认值。
        react.useEffect(() => {
          if (configStore.value.loaded) return;
          api('/dsh-done-sound/api/status')
            .then((payload) => {
              if (payload && typeof payload === 'object') syncConfig(payload);
            })
        ...
        react.useEffect(() => {
          ...
          const notifyPending = () => {
            const canPlay = (view) => {
              const s = (view.sounds && view.sounds.pending) || {};
              return view.enabled && view.playOnPending && (s.url || s.defaultUrl);
            };
            const record = (value) => setConfig({ lastEvent: { at: Date.now(), kind: 'pending', reason: 'pending', play: value } });
            if (canPlay(cfg)) {
              playChime(cfg, 'pending');
              record(true);
              return;
            }
            api('/dsh-done-sound/api/status')
              .then((payload) => {
                ...
                const fresh = {
                  ...
                  enabled: payload.enabled === true,
                  playOnPending: payload.playOnPending !== false,
                  sounds: syncSoundsFrom(payload, cfg),
                };
                if (canPlay(fresh)) {
                  playChime(fresh, 'pending');
                  record(true);
                }
              })
              .catch(() => {});
...
if (snap.pendingCount > 0) {
          ...
          // Each trigger kind maps to its own per-scene audio (normal /
          // interrupt / error); pending and retry scenes are handled by their
          // own paths (notifyPending / fireModuleRetry success).
          const firePlay = (view) => {
            if (kind === 'interrupt') {
              if (view.playOnInterrupt) {
                playChime(view, 'interrupt');
                return true;
              }
              return false;
            }
            if (kind === 'error') {
              if (view.playOnError) {
                playChime(view, 'error');
                return true;
              }
              return false;
            }
            const normalSound = (view) => {
              const s = (view.sounds && view.sounds.normal) || {};
              return s.url || s.defaultUrl;
            };
            playChime(view, 'normal');
            return normalSound(view) ? true : false;
          };

          let played = firePlay(cfg);
          const record = (value) =>
            setConfig({ lastEvent: { at: Date.now(), kind, reason, play: value } });
          const normalSoundMissing = () => {
            const s = (cfg.sounds && cfg.sounds.normal) || {};
            return !s.url && !s.defaultUrl;
          };
          if (!played && normalSoundMissing()) {
        ...
        if (cooldownLeft > 0) {
          ...
          return react.createElement(
            ...
            react.createElement(
              ...
              {
                type: 'button',
                className: 'dtc-retry-cancel',
                title: t(chipLang, 'cancelRetry'),
                onClick: cancelModuleRetryFromCard,
              },
              t(chipLang, 'cancelRetry'),
            ),
          );
        }
        return null;
