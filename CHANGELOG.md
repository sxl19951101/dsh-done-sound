...
- style: 设置卡片五类音效行与开关行对齐排版——场景名固定宽度、文件名自适应省略、按钮组等宽靠右；开关说明统一左对齐

## [0.1.10] - 2026-08-29

- fix: 将「终止重试」按钮放到主界面自动重连倒计时后方，点击后立即取消重试并隐藏倒计时和按钮
- fix: 自动重连成功后，设置卡片显示专属成功音效的播放状态

## [0.1.9] - 2026-08-27

- feat: 五种触发场景可分别配置音频：正常完成、中断、出错、等待确认、自动重连成功
- feat: 每种场景新增独立内置默认音效；未上传自定义音频时也能正常提示
- feat: 新增「重连成功也响」开关，自动发送「继续」成功后播放专属音效
- feat: 兼容旧版本单音频配置，并将旧音频作为正常完成音迁移使用
- feat: 设置卡片支持逐场景选择、试听和清除音频
