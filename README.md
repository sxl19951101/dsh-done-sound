...
- **设置卡片**（设置 → 对话完成音效）：每种触发场景独立选择音频文件（mp3 / wav / ogg / webm / m4a / aac / flac，≤10MB）、试听、清除、音量调节。
- **每种场景独立音频 + 内置默认音**：正常完成、中断、出错、等待确认、自动重连成功共 5 种场景，各自可配不同音频；即使完全不设置，每个场景也有自带的默认提示音，不会无声。
...
- **触发开关**：
  ...
  - `重连成功也响`（默认**开**）：自动发送「继续」成功后是否播放提示音。
- **确认等待提醒**：对话中出现等待确认 / 提问时播放提示音（可独立选音）。
